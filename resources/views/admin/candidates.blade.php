...
<div class="mb-4 px-4 py-3 rounded-xl text-sm bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800/40 text-yellow-800 dark:text-yellow-300">
    No election exists yet. <a href="{{ route('admin.elections.index') }}" class="underline font-semibold">Create one</a> before adding candidates.
</div>
@endif
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-100 dark:divide-white/10">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                ...
                <tr class="hover:bg-gray-50 hover:dark:bg-white/5 transition-colors" id="cand-row-{{ $c->id }}">
                    <td class="px-6 py-4">
                        @if($c->image_url)
                            <img src="{{ $c->image_url }}" class="h-10 w-10 rounded-full object-cover border border-gray-200 dark:border-white/10">
                        @else
                    ...
                    <td class="px-6 py-4 font-medium text-sm text-primary dark:text-parchment">{{ $c->name }}</td>
                    <td class="px-6 py-4 text-sm text-gray-600 dark:text-white/70">{{ $c->party_list ?? '—' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-600 dark:text-white/70">{{ $c->position->name ?? '—' }}</td>
