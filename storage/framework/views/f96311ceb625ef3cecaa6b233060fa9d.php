...
<div class="flex flex-wrap items-center justify-between gap-3 mb-4">
    <div class="flex items-center gap-2">
        ...
        <select id="election-select" onchange="location.href='<?php echo e(route('admin.results.index')); ?>?election=' + this.value"
            ...
            <?php $__empty_1 = true; $__currentLoopData = $elections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <option value="<?php echo e($e->id); ?>" <?php echo e($election && $election->id === $e->id ? 'selected' : ''); ?>>
                    <?php echo e($e->title); ?> (<?php echo e($e->status); ?>)
                </option>
...
<div class="flex flex-wrap items-center justify-between gap-3 mb-6">
    ...
    <div class="flex flex-wrap gap-2">
        <a href="<?php echo e(route('admin.results.export.csv')); ?>?election=<?php echo e($election?->id); ?>"
        ...
        <a href="<?php echo e(route('admin.results.export.pdf')); ?>?election=<?php echo e($election?->id); ?>" target="_blank"
           class="border border-primary text-primary hover:bg-primary hover:text-white dark:border-white/20 dark:text-white dark:hover:bg-white/10 text-sm font-semibold px-4 py-2 rounded-xl transition-colors flex items-center gap-1.5">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="h-4 w-4">
        ...
        <?php if($election && !$election->isClosed()): ?>
...
<?php if($election && $election->isResultsLocked()): ?>
<div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/40 rounded-xl p-4 mb-6 flex flex-wrap items-center justify-between gap-3">
    <div class="flex items-center gap-2 text-sm">
        ...
        <span class="text-green-600 dark:text-green-400">— <?php echo e($election->merkle_leaf_count); ?> ballot(s) sealed into the Merkle root on <?php echo e($election->results_locked_at->format('M j, Y g:i A')); ?>.</span>
    </div>
    <span class="font-mono text-xs text-green-700 dark:text-green-300 bg-white dark:bg-[#0f2a4a] border border-green-200 dark:border-green-800/40 rounded-lg px-2 py-1" title="<?php echo e($election->merkle_root); ?>">
        <?php echo e(substr($election->merkle_root, 0, 10)); ?>…<?php echo e(substr($election->merkle_root, -10)); ?>
...
<?php $__env->startPush('scripts'); ?>
<script>
const resultsElectionId = <?php echo e($election?->id ?? 'null'); ?>;
...
async function loadResults() {
    ...
    try {
        const response = await fetch('<?php echo e(route("admin.results.index")); ?>?election=' + resultsElectionId, {
            headers: {
...
async function confirmReset() {
    ...
    try {
        const response = await fetch('<?php echo e(route("admin.results.reset")); ?>?election=' + resultsElectionId, {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\JULFAHAD_SSG_EVOTING\resources\views/admin/results.blade.php ENDPATH**/ ?>