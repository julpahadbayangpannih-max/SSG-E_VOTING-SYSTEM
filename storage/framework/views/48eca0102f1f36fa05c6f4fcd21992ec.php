...
<?php $__env->startSection('page-subtitle', 'Create new elections and manage past ones — full history is kept.'); ?>
<?php $activeView = 'elections'; ?>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-100 dark:divide-white/10">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                <?php $__empty_1 = true; $__currentLoopData = $elections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-gray-50 hover:dark:bg-white/5 transition-colors <?php echo e($current && $current->id === $e->id ? 'bg-blue-50 dark:bg-blue-900/30' : ''); ?>" id="election-row-<?php echo e($e->id); ?>">
                    <td class="px-6 py-4">
                        <span class="font-medium text-sm text-primary dark:text-parchment"><?php echo e($e->title); ?></span>
                        <?php if($current && $current->id === $e->id): ?>
                    ...
                    <td class="px-6 py-4">
                        <span class="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold uppercase
                            <?php echo e($e->status === 'open' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : ($e->status === 'closed' ? 'bg-gray-200 dark:bg-white/10 text-gray-600 dark:text-white/70' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300')); ?>">
                            <?php echo e($e->status); ?>

                        </span>
                    </td>
                    <td class="px-6 py-4 text-xs text-gray-500 dark:text-white/50">
                        <?php echo e($e->start_time ? $e->start_time->format('M j, Y g:i A') : '—'); ?>

                        <br>to <?php echo e($e->end_time ? $e->end_time->format('M j, Y g:i A') : '—'); ?>

                    </td>
                    <td class="px-6 py-4 text-center text-xs text-gray-500 dark:text-white/50">
                        <?php echo e($e->positions_count); ?> / <?php echo e($e->candidates_count); ?> / <?php echo e($e->votes_count); ?>

                    </td>
                    <td class="px-6 py-4 text-center space-x-2 whitespace-nowrap">
                        <?php if(!($current && $current->id === $e->id)): ?>
                        <button onclick="switchElection(<?php echo e($e->id); ?>)" class="text-blue-600 dark:text-gold-bright hover:text-blue-800 hover:dark:text-gold-bright text-xs font-semibold">Manage</button>
                        <?php endif; ?>
                        <?php if($e->status === 'draft'): ?>
                        <button onclick="editElection(<?php echo e(json_encode(['id'=>$e->id,'title'=>$e->title,'start_time'=>optional($e->start_time)->format('Y-m-d\TH:i'),'end_time'=>optional($e->end_time)->format('Y-m-d\TH:i')])); ?>)" class="text-blue-600 dark:text-gold-bright hover:text-blue-800 hover:dark:text-gold-bright text-xs font-semibold">Edit</button>
                        <button onclick="openElection(<?php echo e($e->id); ?>, '<?php echo e(addslashes($e->title)); ?>')" class="text-green-600 dark:text-green-400 hover:text-green-800 hover:dark:text-green-300 text-xs font-semibold">Open</button>
                        <?php endif; ?>
                        <?php if($e->status === 'open'): ?>
                        <button onclick="closeElection(<?php echo e($e->id); ?>, '<?php echo e(addslashes($e->title)); ?>')" class="text-orange-600 hover:text-orange-800 text-xs font-semibold">Close</button>
                        <?php endif; ?>
                        <?php if($e->votes_count == 0): ?>
                        <button onclick="deleteElection(<?php echo e($e->id); ?>, '<?php echo e(addslashes($e->title)); ?>')" class="text-red-500 dark:text-red-400 hover:text-red-700 hover:dark:text-red-300 text-xs font-semibold">Delete</button>
                        <?php endif; ?>
                    </td>
                </tr>
...
async function saveElection() {
    ...
    const url = id ? `/admin/elections/${id}` : '<?php echo e(route("admin.elections.store")); ?>';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\JULFAHAD_SSG_EVOTING\resources\views/admin/elections.blade.php ENDPATH**/ ?>