...
<div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8 stagger-group">
    ...
    <a href="<?php echo e($card['route']); ?>" class="block bg-white dark:bg-[#0f2a4a] rounded-2xl shadow-sm dark:shadow-black/30 p-5 border-l-4 <?php echo e($card['color']); ?> dark:border-secondary hover:-translate-y-1 hover:shadow-lg transition-all duration-300 ease-out">
        <div class="text-primary/70 dark:text-gold-bright mb-1"><?php echo $card['icon']; ?></div>
        <div class="text-3xl font-extrabold text-primary dark:text-parchment stat-value" data-target="<?php echo e($card['value']); ?>" id="stat-card-<?php echo e($i); ?>">0</div>
        <div class="text-xs text-gray-500 dark:text-white/50 mt-1"><?php echo e($card['label']); ?></div>
    </a>
...
<div class="foil-edge bg-white dark:bg-[#0f2a4a] rounded-2xl shadow-sm dark:shadow-black/30 p-6 mb-6 border border-parchment-line dark:border-white/10">
    <div class="flex items-center justify-between mb-3">
        ...
        <span class="text-2xl font-extrabold text-primary dark:text-gold-bright"><?php echo e($stats['turnout_percent']); ?>%</span>
    </div>
    ...
    <p class="text-xs text-gray-400 dark:text-white/50 mt-2"><?php echo e($stats['voted_count']); ?> out of <?php echo e($stats['total_voters']); ?> approved voters have cast their ballot.</p>
</div>
...
<div class="bg-white dark:bg-[#0f2a4a] rounded-2xl shadow-sm dark:shadow-black/30 p-6 mb-6 border border-parchment-line dark:border-white/10">
    ...
    <?php if($turnoutByCourse->isEmpty()): ?>
...
<div class="bg-white dark:bg-[#0f2a4a] rounded-2xl shadow-sm dark:shadow-black/30 p-6 mb-8 border border-parchment-line dark:border-white/10">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="<?php echo e(route('admin.elections.index')); ?>" class="text-xs font-semibold text-secondary dark:text-gold-bright hover:text-primary dark:hover:text-white">Manage Elections →</a>
    </div>
    <?php if($election): ?>
        <div class="grid sm:grid-cols-3 gap-4 mb-4 text-sm">
            <div>
                ...
                <span class="font-semibold text-primary dark:text-parchment"><?php echo e($election->title); ?></span>
                <span class="ml-2 inline-block px-2 py-0.5 rounded-full text-[10px] font-bold uppercase
                    <?php echo e($election->status === 'open' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : ($election->status === 'closed' ? 'bg-gray-200 text-gray-600 dark:bg-white/10 dark:text-white/60' : 'pulse-attention bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300')); ?>">
                    <?php echo e($election->status); ?>

                </span>
            </div>
            <div>
                <span class="block text-xs font-semibold text-gray-500 dark:text-white/40 uppercase tracking-wide mb-1">Start</span>
                <span class="text-primary dark:text-parchment"><?php echo e($start ? \Carbon\Carbon::parse($start)->format('M j, Y g:i A') : '—'); ?></span>
            </div>
            <div>
                <span class="block text-xs font-semibold text-gray-500 dark:text-white/40 uppercase tracking-wide mb-1">End</span>
                <span class="text-primary dark:text-parchment"><?php echo e($end ? \Carbon\Carbon::parse($end)->format('M j, Y g:i A') : '—'); ?></span>
            </div>
        </div>
        <?php if(!$election->isClosed()): ?>
        ...
        <?php endif; ?>
    <?php else: ?>
        <p class="text-sm text-gray-400 dark:text-white/40">No election created yet. <a href="<?php echo e(route('admin.elections.index')); ?>" class="text-secondary dark:text-gold-bright font-semibold">Create one</a> to get started.</p>
    <?php endif; ?>
</div>


<?php $__env->startPush('scripts'); ?>
<script src="<?php echo e(asset('vendor/chartjs/chart-4.5.1.umd.min.js')); ?>"></script>
<?php $__env->stopPush(); ?>

<?php $__env->startPush('scripts'); ?>
<script>
<?php if($turnoutByCourse->isNotEmpty()): ?>
new Chart(document.getElementById('turnoutByCourseChart'), {
    type: 'bar',
    data: {
        labels: <?php echo json_encode($turnoutByCourse->pluck('course'), 15, 512) ?>,
        datasets: [
            {
                label: 'Voted',
                data: <?php echo json_encode($turnoutByCourse->pluck('voted'), 15, 512) ?>,
                backgroundColor: '#001f3f',
                borderRadius: 6,
            },
            {
                label: 'Not Yet Voted',
                data: <?php echo json_encode($turnoutByCourse->map(fn($c) => $c['total'] - $c['voted']), 15, 512) ?>,
            ...
            tooltip: {
                callbacks: {
                    afterBody: (items) => {
                        ...
                        const pct = <?php echo json_encode($turnoutByCourse->pluck('percent'), 15, 512) ?>[i];
                        return `Turnout: ${pct}%`;
                    }
                }
            }
...
async function confirmReset(btn) {
    ...
    const res = await withButtonLoading(btn, 'Resetting…', () => apiFetch('<?php echo e(route("admin.results.reset")); ?>', { method: 'POST' }));
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\JULFAHAD_SSG_EVOTING\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>