...
<div x-data="darkMode" class="min-h-screen bg-gray-50 dark:bg-ink transition-colors duration-200">
    ...
    <header class="seal-weave bg-primary dark:bg-ink dark:border-b dark:border-white/10 text-white shadow-lg sticky top-0 z-20">
        <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
            <div class="flex items-center space-x-3">
                <img src="<?php echo e($brand['logo_url'] ?? asset('images/SSG.jpg')); ?>" alt="<?php echo e($brand['school_short_name'] ?? 'SSG'); ?>" class="h-10 w-10 rounded-full object-cover border-2 border-secondary">
                <div>
                    <div class="font-serif font-semibold text-base leading-none tracking-tight"><?php echo e($brand['school_name'] ?? 'JRMSU Siocon SSG'); ?></div>
                    <div class="text-[10px] text-secondary uppercase tracking-widest mt-1"><?php echo e($brand['tagline'] ?? 'Official Ballot · E-Voting System'); ?></div>
                </div>
            </div>
    ...
    <main class="max-w-5xl mx-auto px-4 py-8">
        ...
        <?php if(session('voted') || $alreadyVoted): ?>
        <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-6 mb-8 text-center">
            ...
            <p class="text-green-600 dark:text-green-400/90 text-sm">Thank you, <?php echo e($voter->name); ?>. Your ballot has been successfully submitted.</p>

            <?php ($displayReceipt = session('receipt_code') ?? $receiptCode ?? null); ?>
            <?php if($displayReceipt): ?>
            <div class="foil-edge stamp-in mt-5 inline-block bg-white dark:bg-white/5 border border-parchment-line dark:border-white/10 rounded-xl px-6 py-4 shadow-md">
                ...
                <div class="font-mono text-xl font-bold text-primary dark:text-white tracking-wider select-all"><?php echo e($displayReceipt); ?></div>
                <p class="text-[11px] text-gray-400 mt-1">Keep this as your proof of participation. It does not reveal who you voted for.</p>
                <a href="<?php echo e(route('verify.show')); ?>?receipt_code=<?php echo e(urlencode($displayReceipt)); ?>"
        ...
        <div class="bg-white dark:bg-white/5 rounded-2xl shadow-xl p-8 text-center border-t-4 border-red-500 mb-8">
            ...
            <p class="text-gray-600 dark:text-gray-400"><?php echo e($status['message']); ?></p>
        </div>
        ...
        <div class="seal-weave foil-edge bg-primary dark:bg-white/5 dark:border dark:border-white/10 rounded-2xl shadow-lg px-6 py-4 mb-6 flex flex-wrap items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-white text-sm font-semibold tracking-wide"><?php echo e($status['election']->title ?? 'Election'); ?> — Voting is Open</div>
                    <?php if($status['election']?->end_time): ?>
                    <div class="text-white/70 text-xs">Closes <?php echo e($status['election']->end_time->format('F j, Y \a\t g:i A')); ?></div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm px-6 py-4 mb-8 flex items-center gap-4">
            <div class="h-12 w-12 rounded-full bg-primary/10 dark:bg-white/10 text-primary dark:text-white flex items-center justify-center font-bold text-lg shrink-0">
                <?php echo e(strtoupper(substr($voter->name, 0, 1))); ?>

            </div>
            <div class="min-w-0">
                <div class="font-semibold text-gray-800 dark:text-white truncate"><?php echo e($voter->name); ?></div>
                <div class="text-xs text-gray-500 dark:text-gray-400 font-mono truncate">
                    <?php echo e($voter->student_id ?? '—'); ?> <?php if($voter->course): ?> · <?php echo e($voter->course); ?> <?php endif; ?>
                </div>
            </div>
        ...
        <form method="POST" action="<?php echo e(route('voter.vote')); ?>" id="ballot-form">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-white/5 rounded-xl shadow-md overflow-hidden border border-gray-100 dark:border-white/10" data-position="<?php echo e($position->name); ?>">
                    <div class="bg-gray-50 dark:bg-white/5 px-6 py-4 border-b border-gray-100 dark:border-white/10 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-primary dark:text-white"><?php echo e($position->name); ?></h3>
                        ...
                        <label class="cursor-pointer group flex h-full">
                            ...
                            <div class="w-full p-4 rounded-xl border-2 border-gray-200 dark:border-white/10 group-hover:border-secondary group-hover:shadow-md
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="font-bold text-gray-800 dark:text-white text-base md:text-lg truncate candidate-name"><?php echo e($candidate->name); ?></div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400 truncate"><?php echo e($candidate->party_list ?? 'Independent'); ?></div>
                                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\JULFAHAD_SSG_EVOTING\resources\views/voter/dashboard.blade.php ENDPATH**/ ?>